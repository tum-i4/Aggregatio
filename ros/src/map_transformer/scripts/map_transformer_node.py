...
def transform():
    ...
    while True:
        if received_map and x_step != 0.0:
            ...
            ##### make sure that the edges of map are all walls i.e. value of 100
            # length of row
            len_row = len(numpy.arange(X_MIN_IN, X_MAX_IN, SIZE))
            len_col = len(numpy.arange(Y_MIN_IN, Y_MAX_IN, SIZE))
            # top row and bottom row
            new_map[0:len_row] = [100] * len_row
            new_map[-len_row:] = [100] * len_row
            #sides
            for r in [0,len_row - 1]:
                for c in range(len_col):
                    new_map[r + c * len_row] = 100
...
def map_transformer():
    ...
    rospy.Subscriber('robot_0/map', OccupancyGrid, callback_get_map)
    rospy.loginfo(rospy.get_caller_id() +
                  "\tMap transformer subscribed to robot_0/map topic")
    rospy.Subscriber('robot_0/map_metadata', MapMetaData,
                     callback_get_map_metadata)
    rospy.loginfo(rospy.get_caller_id() +
                  "\tMap transformer subscribed to robot_0/map_metadata topic")
