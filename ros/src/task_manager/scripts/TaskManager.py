...
from task_manager_msgs.msg import ScheduleItem, ScheduleUpdated, TaskCompleted
...
ALL_COMPLETED_TASKS_TOPIC = '/all_completed_tasks' #publish completed tasks for analysis
...
class TaskManager:
    ...
    def __init__(self, robot_id: str, initial_x: float, initial_y: float):
        ...
        self.robot_num = int(''.join(s for s in self.robot_id if s.isdigit())) #extract number
        ...
        self.all_completed_tasks_pub = None
    ...
    def __init_publishers(self):
        ...
        self.all_completed_tasks_pub = rospy.Publisher(ALL_COMPLETED_TASKS_TOPIC, TaskCompleted, queue_size=1)
    ...
    def delete_task_cb(self, task_id: String) -> None:
        ...
        try:
            ...
            # Publish which robot completed which task for data collection/analysis
            completedTask = TaskCompleted(robot_id=self.robot_num, x=self.schedule[task_id.data].task.x, y=self.schedule[task_id.data].task.y)
            self.all_completed_tasks_pub.publish(completedTask)
